...
- (void)dealloc
{
    NSLog(@"ConditionViewController Dealloc!");
}
...
-(void)loadAnUpdateImageWithIndex:(int )index{
    ...
    __weak __block typeof(_imageViews) weakImageViews = _imageViews;
    dispatch_sync(mainQueue, ^{
        UIImage *image=[UIImage imageWithData:data];
        UIImageView *imageView= weakImageViews[index];
...
-(void)loadImageWithMultiThread{
    ...
    for (int i=0; i<count; ++i) {
        //加载图片
        dispatch_async(globalQueue, ^{
            [weakSelf loadImage:[NSNumber numberWithInt:i]];
        });
    }
}
